...
import { PixelateOverlayProps } from "../../types";
import { ANIMATION } from "../../constants";

/**
 * Component that creates a pixelation effect over an image
 * Used during image analysis to provide visual feedback
 */
const PixelateOverlay: React.FC<PixelateOverlayProps> = ({
  src,
  active,
  pixelSize = ANIMATION.DEFAULT_PIXEL_SIZE,
  className = "",
}) => {
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      function animate(ts: number) {
        ...
        let p = Math.min(elapsed / ANIMATION.PIXELATE_DURATION, 1);
        setProgress(p);
        if (p < 1 && active) {
